...
CS:3820 Spring 2025 Problem Set 1
=================================
...
The last two questions ask you to explore the Github environment that we use
throughout this course.

To test your code you can use the `cabal` tool.  Cabal makes sure that all
dependencies are available.  To start an interactive shell that has the
functions defined in this file available run `cabal repl` in a terminal.
To run the tests for a single question run `cabal run Test 1`.  You can
replace the number `1` with the question you would like to run.
...
The grading of your solutions is done on Github using "Github Actions".
This allows you to see immediately how many questions you solved, and
also provides continuous feedback.

When you accepted the assignment Github created a repository for you.
One feature that Github provides with repositories is the possibility to
open issues.  While issues are normally used to report bugs in software
projects they are also an excelent mechanism for you to ask questions
about the assignment.  For example, it is possible to refer to specific
lines in your code when discussing possible solutions.

-------------------------------------------------------------------------------}

-- 17. Inspect the autograder output.
--
--     Go to the Github repository and then to the Actions tab.  There you find
--     the workflow that ran on your code.  It might be necessary that you first
--     commit and push your changes.  Click on the latest run and then on the
--     "Autograding" box.
--     You will see a the log output of the workflow.  We are interested in the
--     "Run education/autograding@v1" step.  Expand it and read the output.
--     You can see that the autograder runs all tests on your code.
--     Study the output to find the value that you need to assign to the
--     constant `expectedValue`.

expectedValue :: Int
expectedValue = 0

-- 18. Open a Github issue.
--
--     On your Github repository, open a Github issue and assign this issue
--     to all members of the course team.  Our Github accounts are
--     @jmills015 @trungnguyen-uiowa @hansjoergschurr
--     You are free to choose any title and text for your issue, but this is
--     a great opportunity to ask any question you might have about this
--     problem set, or the class in general.
--
--     Note that issues are private to your repository, and other students
--     can't see them.
